	...
	public Facebook(String appId) {
		if (appId == null) {
			throw new IllegalArgumentException(
					"You must specify your application ID when instantiating a Facebook object. See README for details.");
		}
		this.mAppId = appId;
	}

	public void authorize(Activity activity, DialogListener listener) {
		authorize(activity, new String[0], 32665, listener);
	}

	public void authorize(Activity activity, String[] permissions,
			DialogListener listener) {
		authorize(activity, permissions, 32665, listener);
	}
...
//		}
	...
	private void startDialogAuth(Activity activity, String[] permissions) {
		...
		dialog(activity, "oauth", params, new DialogListener() {
			public void onComplete(Bundle values) {
				CookieSyncManager.getInstance().sync();
				Facebook.this.setAccessToken(values.getString("access_token"));
				Facebook.this.setAccessExpiresIn(values.getString("expires_in"));
				if (Facebook.this.isSessionValid()) {
					Util.logd(
							"Facebook-authorize",
							"Login Success! access_token="
									+ Facebook.this.getAccessToken()
									+ " expires="
									+ Facebook.this.getAccessExpires());
					Facebook.this.mAuthDialogListener.onComplete(values);
				} else {
					Facebook.this.mAuthDialogListener
			...
			if (resultCode == -1) {
				...
				if (error != null) {
					if ((error.equals("service_disabled"))
						...
						startDialogAuth(this.mAuthActivity,
								this.mAuthPermissions);
					...
					setAccessExpiresIn(data.getStringExtra("expires_in"));
					if (isSessionValid()) {
						Util.logd("Facebook-authorize",
								"Login Success! access_token="
				...
				if (data != null) {
					...
					this.mAuthDialogListener.onError(new DialogError(data
	...
	public boolean shouldExtendAccessToken() {
		return (isSessionValid())
				&& (System.currentTimeMillis() - this.mLastAccessUpdate >= 86400000L);
	}
	...
	private class TokenRefreshServiceConnection implements ServiceConnection {
		final Messenger messageReceiver = new Messenger(new Handler() {
			public void handleMessage(Message msg) {
				...
				long expiresAt = msg.getData().getLong("expires_in") * 1000L;

				Bundle resultBundle = (Bundle) msg.getData().clone();
				resultBundle.putLong("expires_in", expiresAt);
				if (token != null) {
					...
					if (Facebook.TokenRefreshServiceConnection.this.serviceListener != null) {
						Facebook.TokenRefreshServiceConnection.this.serviceListener
								.onComplete(resultBundle);
					}
				} else if (Facebook.TokenRefreshServiceConnection.this.serviceListener != null) {
					...
					if (msg.getData().containsKey("error_code")) {
						...
						Facebook.TokenRefreshServiceConnection.this.serviceListener
								.onError(new Error(error != null ? error
										: "Unknown service error"));
					}
				}
				Facebook.TokenRefreshServiceConnection.this.applicationsContext
						.unbindService(Facebook.TokenRefreshServiceConnection.this);
			}
		});
		...
		private void refreshToken() {
			...
			try {
				...
				this.serviceListener.onError(new Error(
	...
	public String request(Bundle parameters) throws MalformedURLException,
		...
		if (!parameters.containsKey("method")) {
			throw new IllegalArgumentException(
					"API method must be specified. (parameters must contain key \"method\" and value). See http://developers.facebook.com/docs/reference/rest/");
		}
	...
	public void setAccessExpiresIn(String expiresIn) {
		if (expiresIn != null) {
			long expires = expiresIn.equals("0") ? 0L : System
