...
public class CalendarCard extends RelativeLayout {
	...
	public int curMonth = Calendar.getInstance().getTime().getMonth();
	@SuppressWarnings("deprecation")
	public int curYear = Calendar.getInstance().getTime().getYear();

	private TextView cardTitleMonth;
	private TextView cardTitleYear;
	...
	List<Integer> daysHaveFavoriteWords;
	...
	private void init(Context ctx) {
		if (isInEditMode())
			return;
		layout = LayoutInflater.from(ctx).inflate(R.layout.card_view, null,
				false);

		// Database
		final GlobalVariable globalVariable = (GlobalVariable) layout
				.getContext().getApplicationContext();
		userStatisticDBAdapter = globalVariable.getUserStatisticDBAdapter();
		...
		cardTitleMonth.setText(new SimpleDateFormat("MMM", Locale.getDefault())
				.format(dateDisplay.getTime()));
		cardTitleYear.setText(new SimpleDateFormat("yyyy", Locale.getDefault())
				.format(dateDisplay.getTime()));
	...
	public void updateCells() {
		...
		daysHaveFavoriteWords = userStatisticDBAdapter.getDaysHaveFavoriteWords(month, year);

		int countDaysHaveFavoriteWords = 0;
		for (int i = firstDay; i < lastDay; i++) {
			...
			// setEnable doan phu hop o giua
			cell.setEnabled(true);
			cell.setBackgroundColor(getResources().getColor(
					R.color.daysDoNotHaveBookmark));
			cell.setVisibility(View.VISIBLE);

			if (countDaysHaveFavoriteWords < daysHaveFavoriteWords.size()) {
				if (i == daysHaveFavoriteWords.get(countDaysHaveFavoriteWords)) {
					cell.setBackgroundColor(getResources().getColor(
							R.color.middle_line));
					++countDaysHaveFavoriteWords;
				} else {
